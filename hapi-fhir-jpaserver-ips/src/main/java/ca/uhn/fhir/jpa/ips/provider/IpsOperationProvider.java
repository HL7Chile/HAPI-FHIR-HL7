...
public class IpsOperationProvider {
	...
	public IBaseBundle patientTypeSummary(
			@Description(
				shortDefinition =
						"Cuando no se utiliza el id lógico del paciente, los servidores PUEDEN optar por admitir la selección de pacientes basada en el identificador proporcionado.")
					@OperationParam(name = "profile", min = 0, typeName = "uri") IPrimitiveType<String> theProfile,
			@Description(
							shortDefinition =
									"Cuando no se utiliza el id lógico del paciente, los servidores PUEDEN optar por admitir la selección de pacientes basada en el identificador proporcionado.")
