...
public class IpsGeneratorSvcImpl implements IIpsGeneratorSvc {
	...
	private IBaseBundle generateIpsForPatient(
		...
		composition.getMeta().addProfile(theStrategy.getCompositionProfile());
	...
	private void addSection(
		...
		for (IBaseResource next : theResourcesToInclude.getResources()) {
			...
			sectionBuilder.addEntry(new IdType(next.getIdElement().getValue()));
	...
	private CompositionBuilder createComposition(
		...
		//compositionBuilder.addProfile(theStrategy.getCompositionProfile());
	...
	private String createSectionNarrative(
		...
		// signature.setWho(new Reference().setDisplay("HL7 Chile"));
		// signature.setWhenElement(InstantType.now());
