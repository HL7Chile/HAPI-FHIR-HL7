FROM docker.io/library/maven:3.9.11-amazoncorretto-17 AS build-hapi
WORKDIR /tmp/hapi-fhir-jpaserver-starter

ARG OPENTELEMETRY_JAVA_AGENT_VERSION=1.31.0
RUN curl -LSsO https://github.com/open-telemetry/opentelemetry-java-instrumentation/releases/download/v${OPENTELEMETRY_JAVA_AGENT_VERSION}/opentelemetry-javaagent.jar

COPY hapi-fhir-jpaserver-starter/pom.xml .
COPY hapi-fhir-jpaserver-starter/server.xml .
RUN mvn -ntp dependency:go-offline

COPY hapi-fhir-jpaserver-starter/src/ /tmp/hapi-fhir-jpaserver-starter/src/
